...
export default async function OrderDetailsPage({ params }: { params: { id: string } }) {
  ...
  const timelineSteps = [
    ...
    { title: "Ready for Dispatch", date: "Pending", icon: Truck, completed: order.status === "Ready for Dispatch" || order.status === "Completed" },
    { title: "Completed", date: "Pending", icon: CheckCircle2, completed: order.status === "Completed" },
  ];

  return (
    <div className="space-y-8 max-w-5xl mx-auto p-6 md:p-8 w-full pb-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Placed on {new Date(order.date).toLocaleDateString()} by {order.buyerName}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Current progression of the order.</CardDescription>
            </CardHeader>
            <CardContent>
